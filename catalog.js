(function () {
  ...
  function platformsUnionForGame(g, verKeys) {
    ...
    for (var k = 0; k < platformOrder.length; k++) {
      if (seen[platformOrder[k]]) out.push(platformOrder[k]);
    }
    return out;
  }
  ...
  function applyPlatformFilters() {
    var filtering = activeFilterPlatforms.size > 0;
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var show = false;
      for (var j = 0; j < platformOrder.length; j++) {
        var plat = platformOrder[j];
  ...
  function syncFilterButtonPressedStates() {
    if (!toolbarEl) return;
    var buttons = toolbarEl.querySelectorAll("button.plat-filter");
  ...
  function mountPlatformToolbar() {
    ...
    label.className = "catalog-toolbar-label";
    label.textContent = "Platforms";

    var group = document.createElement("div");
    group.className = "catalog-toolbar-toggles";
    group.setAttribute("role", "group");
    group.setAttribute("aria-label", "Filter catalog by platform");
